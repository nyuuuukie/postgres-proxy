...
static const std::vector<std::string> titles{" CRIT", "ERROR", " INFO", "DEBUG", ""};
...
void Logger::stdoutLogs(bool flag) {
    _stdoutLogs = flag;
}

void Logger::fileLogs(bool flag) {
    if (_fileLogs == flag) {
        return;
    }
...
void Logger::setFilePrefix(const std::string& prefix) {
    _logFilePrefix = prefix;
}

Logger& Logger::print(Levels level) {
    _m_lock_print.lock();
    _askLevel = level;
...
std::ostream& Logger::endl(std::ostream& out) {
    out << std::endl;
    return out;
}

std::ostream& Logger::ends(std::ostream& out) {
    out << std::ends;
    return out;
}